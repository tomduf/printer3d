...
const int epaisseurCouche = 0.1; // mm
...
void loop() {
  ...
  case 2 :                      // down
    ...
    break;
  case 7 :                      // cycle de remontée du plateau
    if (pas != -1){
      vitesse = 100;
  ...
  if (changerTexte)
    ...
    if (mode != 0 && mode != 7 && mode != 9){
      ...
      delay(500);
      mode = 0;
      Wire.write(8); // Envoi de la fin du cycle au serveur
    }
    else if (mode == 7){  // cycle de remontée du plateau
      for (int i = 0; i < 5000 ; i++){
        myStepper.step(pas);        
      }
      delay(500);
      mode = 0;
      Wire.write(6); // Envoi de la fin de remontée du plateau au serveur
    }
}
